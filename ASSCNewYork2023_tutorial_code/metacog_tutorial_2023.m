%% ASSC HMeta-d tutorial
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% * METACOGNITION TUTORIAL 2023*
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% 
% 
% 
% Based on code by M. Rouault, O. Faull and S. Fleming
% 
% 
% 
% *INSTRUCTIONS:*
...
% version.
% There are also R wrappers that interface with the HMeta-d model code,
% and which do not pose a compatibility restriction with JAGS versions 
% This tutorial is in Matlab, but many of the same functions can be
% accessed via R - see https://github.com/metacoglab/HMeta-d/tree/master/R 
% 
% You can type "path" in the Matlab console to check if the HMeta-d-master
% toolbox has been added to your path.
% 
% You will need the Matlab optimization toolbox installed.
% For Extension problem 3, you will also need the Matlab statistical
% toolbox installed.
% 
% 
% 
...
% ----------------------------------------------------------------------------------------------------------------------------------------------------------------
% 
% First, we will simulate data with high and low internal noise, which should 
% lead to higher and lower metacognition. This added noise represents a potential 
% loss of information between type 1 task discrimination and 'metacognitive' information, 
% i.e. type 2 confidence scores. We will then plot what the confidence scores 
% would look like for each of these two scenarios (high and low internal noise).
% 
% 
% 
...
% 
% _FUNCTION:_ cpc2021_type2_SDT_sim(d, noise, c, Nratings, Ntrials);

sim1.lowNoise.responses = cpc2021_type2_SDT_sim(sim1.d, sim1.noise.low, sim1.c, sim1.Nratings, sim1.Ntrials);
sim1.highNoise.responses = cpc2021_type2_SDT_sim(sim1.d, sim1.noise.high, sim1.c, sim1.Nratings, sim1.Ntrials);
%% 
% 
% 
% *Plot the confidence results for the two simulations:*
% 
% _FUNCTION:_ cpc2021_plot_confidence(data, title, type)
% 
...
subplot(1,2,1)
cpc2021_plot_confidence(sim1.lowNoise, 'LOW NOISE', 'obs');
subplot(1,2,2)
cpc2021_plot_confidence(sim1.highNoise, 'HIGH NOISE', 'obs');
%% 
% 
% 
% *EXPLANATION:*
% 
% You can see in the figure that the low internal noise likely results in 
% less overlap between the confidence distributions for correct and incorrect 
...
% metacognition will be. Another way of thinking about it is that the less 'noise' 
...
sim2.dHigh.responses = cpc2021_type2_SDT_sim(sim2.dValueHigh, sim2.noise.value, sim2.c, sim2.Nratings, sim2.Ntrials);
sim2.dLow.responses = cpc2021_type2_SDT_sim(sim2.dValueLow, sim2.noise.value, sim2.c, sim2.Nratings, sim2.Ntrials);
%% 
% 
% 
% *Plot the confidence results for the two simulations:*
...
set(gcf, 'Position', [200 200 800 300])
subplot(1,2,1)
cpc2021_plot_confidence(sim2.dHigh, 'HIGHER d''', 'obs');
subplot(1,2,2)
cpc2021_plot_confidence(sim2.dLow, 'LOWER d''', 'obs');
%% 
% 
% 
% *EXPLANATION:*
...
% amount of internal noise. Later on we will look at how we can correct for the 
% level of d' to get a 'relative' measure of metacognition.
% 
% 
% 
% *EXERCISE 2:*
% 
% ?? Change any of the parameters above and re-run this section to get an 
% idea as to how each parameter may change the observed confidence scores.
% 
% 
% 
% 
...
sim3.lowNoise.responses = cpc2021_type2_SDT_sim(sim3.d, sim3.noise.low, sim3.c, sim3.Nratings, sim3.Ntrials);
sim3.highNoise.responses = cpc2021_type2_SDT_sim(sim3.d, sim3.noise.high, sim3.c, sim3.Nratings, sim3.Ntrials);
%% 
% 
% 
% *Plot the confidence results for the two simulations:*
...
set(gcf, 'Position', [200 200 800 300])
subplot(1,2,1)
cpc2021_plot_confidence(sim3.lowNoise, 'LOW NOISE', 'obs');
subplot(1,2,2)
cpc2021_plot_confidence(sim3.highNoise, 'HIGH NOISE', 'obs');
%% 
% 
% 
% *Now plot the observed type2 data in a type2ROC curve:*
% 
% _FUNCTION:_ cpc2021_plot_type2roc(data, title, type)
% 
% _NOTE:_ type = 'obs' or 'est' (observed or estimated data from model)
...
set(gcf, 'Position', [200 200 800 300])
subplot(1,2,1)
cpc2021_plot_type2roc(sim3.lowNoise, 'LOW NOISE', 'obs');
subplot(1,2,2)
cpc2021_plot_type2roc(sim3.highNoise, 'HIGH NOISE', 'obs');
%% 
% 
% 
% *EXPLANATION:*
...
% that when there is lower internal noise, the type2ROC curve has a larger 'bow' 
...
% 
% _FUNCTION:_ cpc2021_calcAU_type2roc(data)

low_noise_area = cpc2021_calcAU_type2roc(sim3.lowNoise) % note we avoid using ; to end the line here so that the output prints to the matlab command window
high_noise_area = cpc2021_calcAU_type2roc(sim3.highNoise)
%% 
% 
% 
...
% lower internal noise is larger than when the data are produced with higher internal 
% noise. This metric is often used as an absolute measure of metacognition. We 
% can also check what happens to the area under the type2ROC when we have a higher 
% value of d', and no difference in the internal noise... To do this, try calculating 
% the area under the type2ROC for the data we simulated using different levels 
% of d' from step 2. How does higher d' affect the area under the typeROC?
% 
% _HINTS:_
% 
%     - You can plot using: cpc2021_plot_type2roc(sim2.dHigh, 'HIGH d', 'obs')
% 
%     - You can calculate the area using: cpc2021_calcAU_type2roc(sim2.dHigh)
% 
% 
% 
% 
...
% For full details of the model please refer to the paper by Maniscalco and Lau 
...
% 
% _FUNCTION:_ cpc2021_metad_sim(d, meta-d', c, Nratings, Ntrials);

sim4.lowMetad.responses = cpc2021_metad_sim(sim4.d, sim4.metad.low, sim4.c, sim4.Nratings, sim4.Ntrials);
sim4.highMetad.responses = cpc2021_metad_sim(sim4.d, sim4.metad.high, sim4.c, sim4.Nratings, sim4.Ntrials);
%% 
% 
% 
...
% and stimulus 2 (nR_S2) for each confidence level, and are generated by cpc2021_metad_sim
...
figure
cpc2021_plot_confidence(sim4.highMetad, 'HIGH META-D', 'est');
cpc2021_plot_type2roc(sim4.highMetad, 'HIGH META-D', 'est');
%% 
% 
% 
...
figure
cpc2021_plot_confidence(sim4.lowMetad, 'LOW META-D', 'est');
cpc2021_plot_type2roc(sim4.lowMetad, 'LOW META-D', 'est');
%% 
% 
% 
% *EXPLANATION:*
...
% that will help us cope with more complex data.
% 
% 
% 
% *EXERCISE 4:*
...
for a = 1:length(sim5.params.meta_d)
    for b = 1:sim5.params.Nsims
        ...
        % Simulate data
        sim5.data{b,a} = cpc2021_metad_sim(sim5.values{a}.d(b), sim5.values{a}.meta_d(b), sim5.values{a}.c(b), sim5.params.Nratings, sim5.params.Ntrials);
...
% 
% _FUNCTION:_ cpc2021_plot_simVfit(data, data.fit, parameter, model)
% 
% _NOTE:_ parameter = 'meta-d' or 'Mratio'
% 
% _NOTE:_ model = 'single' or 'group' (model used, here it is 'single')

cpc2021_plot_simVfit(sim5, sim5.fit, 'meta-d', 'single');
%% 
% 
% 
% *EXPLANATION:*
...
% group value. We will now repeat the exercise from step 5 using this
% model, now with 100 trials per simulated subject.
% 
% 
% 
% *Set up the parameters:*
...
for a = 1:length(sim6.params.meta_d)
    for b = 1:sim6.params.Nsims
        ...
        sim6.data{b,a} = cpc2021_metad_sim(sim6.params.d, sim6.params.meta_d(a), sim6.params.c, sim6.params.Nratings, sim6.params.Ntrials);
...
cpc2021_plot_simVfit(sim6, sim6.fit, 'meta-d', 'group');
%% 
% 
% 
% *EXPLANATION:*
...
for a = 1:2
    for b = 1:simTwoGroupsI.params.Nsims(a)
        ...
        % Simulate data
        simTwoGroupsI.data{a}.sims{b} = cpc2021_metad_sim(simTwoGroupsI.values{a}.d(b), simTwoGroupsI.values{a}.meta_d(b), simTwoGroupsI.values{a}.c(b), simTwoGroupsI.params.Nratings, simTwoGroupsI.params.Ntrials);
...
% 
% Run the exercise again, but this time create two groups with a very small (e.g. 
...
% ----------------------------------------------------------------------------------------------------------------------------------------------------------------
% 
% We will now go through an example where we simulate and fit data from two dependent 
...
for b = 1:simTwoGroupsD.params.Nsims
    ...
    for a = 1:2
        ...
        % Simulate data
        simTwoGroupsD.data{a}.sims{b} = cpc2021_metad_sim(simTwoGroupsD.values{a}.d(b), simTwoGroupsD.values{a}.metad(b), simTwoGroupsD.values{a}.c(b), simTwoGroupsD.params.Nratings, simTwoGroupsD.params.Ntrials);
...
for a = 1:simRegression.params.Nsims
    simRegression.data{a} = cpc2021_metad_sim(simRegression.values.d(a), simRegression.values.meta_d_adjusted(a), simRegression.values.c(a), simRegression.params.Nratings, simRegression.params.Ntrials);
    simRegression.responses.nR_S1{a} = simRegression.data{a}.nR_S1;
    simRegression.responses.nR_S2{a} = simRegression.data{a}.nR_S2;
end
%% 
% 
% first check if statistical toolbox is installed for fitlm:
...
cpc2021_plot_simVfit(simRegression, simRegression.fit, 'log(Mratio)', 'regression');
%% 
% 
% 
...
simRC.params.noise = [0.2 0.8]; % % Specify the internal noise to be different between the two stimuli
%% 
% 
% 
...
for a = 1:simRC.params.Nsims
    simRC.data{a} = cpc2021_type2_SDT_sim(simRC.values.d(a), simRC.params.noise, simRC.values.c(a), simRC.params.Nratings, simRC.params.Ntrials);
